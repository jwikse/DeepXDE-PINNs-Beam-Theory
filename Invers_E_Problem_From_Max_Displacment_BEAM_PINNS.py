...
# Nondimensionalization
def w_hat_analytic(x_hat):  
    xi = x_hat
    return (xi**4)/24 - (xi**3)/6 + (xi**2)/4
...
E_hat = dde.Variable(1.5)  # initial guess

# Dimensionless PDE: E_hat * w'''' - 1 = 0
def pde_hat(x, y):
...
x_max_location = np.array([[1.0]])
w_max_true_hat = w_hat_analytic(1.0)  # True max displacement (non-dimensional)

# Max displacement constraint
bc_max_disp = dde.PointSetBC(x_max_location, w_max_true_hat, component=0)


bcs = [ bc_slope_left, bc_moment_right,bc_shear_right,bc_max_disp,]
...
# Plot prediction (physical units)
