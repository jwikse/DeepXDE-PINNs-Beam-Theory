...
bc_sheer_right = dde.OperatorBC(geom, sheer_bc, bc_right)

bcs = [bc_rotation_left, bc_moment_right, bc_sheer_right]
...
# Analytical Solution (cantilever with uniform force, non-dimensionalized)
def func(x):
    return (q_hat*(x**4 - 4*L_hat*x**3 + 6*L_hat**2*x**2)) / (24*E_hat*I_hat)

# Predict in non-dimensional domain
x_values = np.linspace(0, 1, 100)
w_pred_nondim = model.predict(x_values.reshape(-1, 1))
w_exact_nondim = func(x_values)
...
plt.semilogy(train_loss[:, 2], label='BC Right (Sheer)', linewidth=2)
